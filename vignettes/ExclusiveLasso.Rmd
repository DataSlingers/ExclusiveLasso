...
**ExclusiveLasso GLMs are currently buggy!**
...
| Family   | Negative Log-Likelihood | Gradient |
|---------:|:------------------------|:---------|
...
\[l_i(\eta_i) = b(\eta_i) - y_i * \eta_i\]

so 

\[\frac{\partial l_i}{\partial \eta_i} = b'(\eta_i) - y_i\]

and 

\[\frac{\partial^2 l_i}{\partial \eta_i^2} = b''(\eta_i)\]

For the GLM families we considered above, we have: 

- Gaussian: $b(x) = \frac{1}{2}x^2$ so $b'(x) = x$ and $b''(x) = 1$, giving 
  \[\frac{\partial l_i}{\partial \eta_i} = \eta_i - y_i = \bx_i^T\beta + \alpha + o_i - y_i\] and 
  \[\frac{\partial^2 l_i}{\partial \eta_i^2} = 1\] 
  as we would expect
- Logistic: $b(x) = \log(1+e^x)$ gives $b'(x) = (1 + e^{-x})^{-1}$ and 
  $b''(x) = b'(x)(1 - b'(x))$ so
  \[\frac{\partial l_i}{\partial \eta_i} = \frac{1}{1+e^{-\eta_i}} - y_i\] and 
  \[\frac{\partial^2 l_i}{\partial \eta_i^2} = \left(1 - \frac{1}{1+e^{-\eta_i}}\right)\left(\frac{1}{1+e^{-\eta_i}}\right)\] 
- Poisson: $b(x) = \exp(x)$ gives $b'(x) = b''(x) = \exp(x)$ so 
  \[\frac{\partial l_i}{\partial \eta_i} = e^{\eta_i} - y_i\] and 
  \[\frac{\partial^2 l_i}{\partial \eta_i^2} = e^{\eta_i}\] 
  
Note that these working residuals and weight adjustments are used for the IRLS-type
scheme used to fit the GLM, so we do not include the observation weights and $n^{-1}$
factor in their calculation. Observation weights and $n^{-1}$ are included in the
coordinate descent inner loop directly in the same fashion that they are for
Gaussian models.
...
1. Initialize
    ...
    - $z(\eta^{(0)})_j =\bo - \frac{\partial l_j / \partial \eta_j}{\partial^2 l_j / \partial \eta_j^2}$
    - $\omega_j^{(0)} = \frac{\partial^2 l_j}{\partial \eta_j^2} * w_j$
    - $\br = z(\eta^{(0)}) - (X\beta + \alpha^{(0)} + \bo) = z(\eta^{(0)}) - \bo$
    - $k = 1$
2. Repeat Until Convergence:
    - Coordinate descent loop -- repeat until convergence: 
    ...
    - Update quadratic approximation:
        ...
        - $z(\eta^{(k)})_j = \eta^{(k)}_j - \frac{\partial l_j/ \partial \eta_j}{\partial^2 l_j / \partial \eta_j^2}$
        - $\omega_j^{(k)} = w_j * \frac{\partial^2 l_j}{\partial \eta_j^2}$
...
algorithm above, however, after each coordiante descent loop, we update the
weights $\Omega$, as well as the working response $z$ and residuals. We only
progress to the next value of $\lambda$ when the outer (quadratic approximation)
loop converges. 
