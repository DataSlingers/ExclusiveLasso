...
2. Repeat until convergence: (outer proximal Newton loop)
    ...
    - Repeat: (proximal Newton backtracking search)
        ...
        - If^[Note that we omit the gradient with respect to $\alpha$ from the RHS of the 
              descent check. Since this makes the check (slightly) more rigorous, this
              is not a problem.]
            \[f(\beta_{\text{new}}, \alpha_{\text{new}}) < f(\beta_{\text{previous}}, \alpha_{\text{previous}}) + 0.5 \left(t * \langle \nabla_{\beta = \beta_{\text{previous}}}\ell, \Delta \beta\rangle + \lambda P(\beta_{\text{new}}) - \lambda P(\beta_{\text{previous}})\right):\]
            - $\texttt{break}$
        - Else: 
